...
public abstract class LiquidizerMenu extends AbstractContainerMenu
{
    ...
    public LiquidizerMenu(MenuType<LiquidizerMenu> type, int containerId, Inventory inventory, BlockEntity blockEntity)
    {
		...
		this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.DOWN).ifPresent(handler -> {
			this.addSlot(new SlotItemHandler(handler, 0, 126, 58));
		});
    }
    ...
    private boolean hasRequiredLiquid(ItemStack itemStack) {
        return itemStack.getCapability(ForgeCapabilities.FLUID_HANDLER_ITEM).map(cap ->
                cap.getFluidInTank(0).getFluid().isSame(blockEntity.getDesiredFluid1())
        ).orElse(false);
    }
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) 
    {
        ...
        if (index < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) 
        {
            ...
            else if (Items.BUCKET.equals(sourceSlot.getItem().getItem())) {
